...
export async function createEvent(name,client=''){const id='e'+Math.floor(Date.now()+Math.random()*1e6).toString(36);const meta={name:name||'新活動',client,listed:true};await FB.patch(`/events/${id}/meta`,meta);await FB.put(`/events/${id}/info`,{title:meta.name,client,dateTime:'',venue:'',address:'',mapUrl:'',bus:'',train:'',parking:'',notes:'',hintTitle:'活動提示',hintText:'',labelPhone:'電話',labelCode:'代號',labelDept:'部門'});await FB.put(`/events/${id}/people`,[]);await FB.put(`/events/${id}/prizes`,[]);await FB.put(`/events/${id}/currentPrizeId`,null);await FB.put(`/events/${id}/questions`,[]);await FB.put(`/events/${id}/photos`,[]);await FB.put(`/events/${id}/logo`,"");await FB.put(`/events/${id}/banner`,"");await FB.put(`/events_index/${id}`,meta);return id;}
