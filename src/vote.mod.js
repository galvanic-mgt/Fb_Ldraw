...
async function boot(){
  ...
  eid = q.event || '';
  pid = q.poll || '';
  if (!eid || !pid) { els.title.textContent = '找不到投票'; els.subtitle.textContent='連結缺少參數'; els.btn.style.display='none'; return; }

  const p = await FB.get(`/events/${eid}/polls/${pid}`);
  if (!p) { els.title.textContent='投票未就緒'; els.subtitle.textContent='請稍後再試'; els.btn.style.display='none'; return; }
  renderPoll(p);
